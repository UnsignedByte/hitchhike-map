...
export async function init (
  ...
  functions[`player_facing_npc`] = ((): string[] =>{
    ...
    for(let i: number = data.npc.params.facing_res; i > 0; i--) {
      ...
      cmds.push(`execute at @s anchored eyes positioned ^ ^1 ^${td} run tag @e[tag=npc,distance=..${factor*data.npc.params.leniency}] add player_facing_npc`);
      cmds.push(`execute at @s anchored eyes positioned ^ ^-1 ^${td} run tag @e[tag=npc,distance=..${factor*data.npc.params.leniency}] add player_facing_npc`);
      cmds.push(`execute at @s anchored eyes positioned ^ ^ ^${td} run particle dust 1 0 0 1 ~ ~ ~`);
      cmds.push(`execute at @s anchored eyes positioned ^ ^1 ^${td} run particle dust 1 0 0 1 ~ ~ ~`);
      cmds.push(`execute at @s anchored eyes positioned ^ ^-1 ^${td} run particle dust 1 0 0 1 ~ ~ ~`);
    }
    return [
      ...cmds,
