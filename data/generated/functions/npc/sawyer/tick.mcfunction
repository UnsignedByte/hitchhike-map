...
execute unless score dialogue-begun dialogue-status matches 1 store success score dialogue-begun dialogue-status if entity @a[tag=victim-of-dialogue-by-sawyer, tag=!spoken-to, limit=1] if score @s dialogue-status matches 9 run schedule function generated:npc/sawyer/8-0 1t
