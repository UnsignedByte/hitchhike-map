...
execute unless score dialogue-begun dialogue-status matches 1 store success score dialogue-begun dialogue-status if entity @a[tag=victim-of-dialogue-by-stephen, tag=!spoken-to, limit=1] if score @s dialogue-status matches 3 run schedule function generated:npc/stephen/2-0 1t
