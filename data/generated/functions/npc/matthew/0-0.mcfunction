...
execute at @e[type=villager,tag=npc-matthew, limit=1] run tellraw @a[tag=npc-eavesdropper] ["<",{"text":"Matthew Sciento","bold":true},"> ",{"text":"As the emerging statistical models are clearly revealing, making these steps (provided, in the right direction) can significantly improve our quarter performance."}]
