...
execute at @e[tag=npc-apple_sale, limit=1] run tellraw @a[distance=..50] ["<",{"text":"Glow Berry (${toCost(item.store.unsold.berry.tag.cost)})","color":"#fad352","bold":true},"> ",{"text":"taken"}]
