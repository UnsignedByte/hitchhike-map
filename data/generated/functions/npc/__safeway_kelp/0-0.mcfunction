...
execute at @e[tag=npc-__safeway_kelp, limit=1] run tellraw @a[tag=npc-eavesdropper] ["<",[{"text":"Fresh Seaweed","bold":false,"color":"#3bbd17"},{"text":" (1.19B)","bold":false,"italic":false,"strikethrough":false,"obfuscated":false,"underlined":false}],"> ",{"text":"taken"}]
