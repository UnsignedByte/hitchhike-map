...
execute at @e[tag=npc-__safeway_pie, limit=1] run tellraw @a[tag=npc-eavesdropper] ["<",[{"text":"Pumpkin Pie","bold":false,"color":"#d66c0f"},{"text":" (8.99B)","bold":false,"italic":false,"strikethrough":false,"obfuscated":false,"underlined":false}],"> ",{"text":"taken"}]
