...
execute unless score dialogue-begun dialogue-status matches 1 store success score dialogue-begun dialogue-status if entity @a[tag=victim-of-dialogue-by-fountainman, tag=!spoken-to, limit=1] if score @s dialogue-status matches 0 run schedule function generated:npc/fountainman/0-0 1t
