...
execute at @e[type=villager,tag=npc-solomon, limit=1] run tellraw @a[tag=npc-eavesdropper] ["<",{"text":"Jason Quint","color":"#b68bdd","bold":true},"> ",{"text":"Imagine if I let you get the key to the electrical box from the chest in the basement, climb up that tower, and get hurt doing so!"}]
