...
execute at @e[tag=npc-janitor, limit=1] run tellraw @a[tag=npc-eavesdropper] ["<",{"text":"Ivan Cummings","color":"#f2d811","bold":true},"> ",{"text":"Well, don't dawdle any longer! Stop wasting my time and get on with it!"}]
...
give @a[tag=victim-of-dialogue-by-janitor, limit=1] minecraft:iron_shovel{display: {Name: '{"text":"Shovel","color":"gray"}', Lore: ['{"text":"Literally just a shovel.","color":"dark_gray"}']}, Unbreakable: 1b, HideFlags: 127, CanDestroy: ["minecraft:snow","minecraft:snow_block","minecraft:powder_snow"]} 1
