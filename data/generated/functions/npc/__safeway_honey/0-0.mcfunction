...
execute at @e[tag=npc-__safeway_honey, limit=1] run tag @a[distance=..50] add npc-eavesdropper
# Dialogue line #0-1: speak and make noise.
execute at @e[tag=npc-__safeway_honey, limit=1] run tellraw @a[tag=npc-eavesdropper] ["<",[{"text":"Honey","bold":false,"color":"#e8c733"},{"text":" (8.99B)","bold":false,"italic":false,"strikethrough":false,"obfuscated":false,"underlined":false}],"> ",{"text":"taken"}]
